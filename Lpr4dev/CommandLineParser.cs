    ...
    public static class CommandLineParser
    {
        public static MapOptions<CommandLineOptions> TryParseCommandLine(IEnumerable<string> args, bool isDesktopApp)
        {
            ...
            OptionSet options = new OptionSet
            {
                ...
                { "hostname=", "Specifies the server hostname. Used in auto-generated TLS certificate for LPR if enabled.", data => map.Add(data, x => x.ServerOptions.HostName) },
                { "allowremoteconnections", "Specifies if remote connections will be allowed to the LPR and IMAP servers. Use -allowremoteconnections+ to enable or -allowremoteconnections- to disable", data => map.Add((data !=null).ToString(), x => x.ServerOptions.AllowRemoteConnections) },
                { "disableipv6", "If true, LPR and IMAP servers will NOT listen using IPv6 Dual Stack", data => map.Add((data !=null).ToString(), x => x.ServerOptions.DisableIPv6)},
                { "smtpport=", "Set the port the LPR server listens on. Specify 0 to assign automatically", data => map.Add(data, x => x.ServerOptions.Port) },
                ...
                { "tlsmode=", "Specifies the TLS mode to use for LPR. None=Off. StartTls=On demand if client supports STARTTLS. ImplicitTls=TLS as soon as connection is established.", data => map.Add(data, x => x.ServerOptions.TlsMode) },
                { "tlscertificate=", "Specifies the TLS certificate file to use for LPR if TLS is enabled/requested. This must be an X509 certificate - generally a .CER, .CRT or .PFX file. If using .CER or .CRT, you must provide the private key separately using --tlscertificateprivatekey.  Specify \"\" to use an auto-generated self-signed certificate (then see console output on first startup).", data => map.Add(data, x => x.ServerOptions.TlsCertificate) },
                { "tlscertificateprivatekey=", "Specifies the corresponding private key file for the LPR TLS certificate if the private key is not part of the TlsCertificate file", data => map.Add(data, x => x.ServerOptions.TlsCertificatePrivateKey) },
                { "tlscertificatepassword=", "Specifies the password for  LPR TLSCertificate/TlsCertificatePrivateKey", data => map.Add(data, x => x.ServerOptions.TlsCertificatePassword) },
                { "relaysmtpserver=", "Sets the name of the LPR server that will be used to relay messages or \"\" if messages relay should not be allowed", data => map.Add(data, x => x.RelayOptions.SmtpServer) },
                { "relaysmtpport=", "Sets the port number for the LPR server used to relay messages", data => map.Add(data, x => x.RelayOptions.SmtpPort) },
                ...
                { "relayusername=", "The username for the LPR server used to relay messages. If \"\" no authentication is attempted", data => map.Add(data, x => x.RelayOptions.Login) },
                { "relaypassword=", "The password for the LPR server used to relay messages", data => map.Add(data, x => x.RelayOptions.Password) },
                { "relaytlsmode=",  "Sets the TLS mode when connecting to relay LPR server. See: http://www.mimekit.net/docs/html/T_MailKit_Security_SecureSocketOptions.htm", data => map.Add(data, x => x.RelayOptions.TlsMode) },
                ...
                { "authenticationrequired", "Requires that LPR and IMAP clients authenticate", data => map.Add((data != null).ToString(), x => x.ServerOptions.AuthenticationRequired) },
                { "secureconnectionrequired", "Requires that LPR clients use SSL/TLS", data => map.Add((data != null).ToString(), x => x.ServerOptions.SecureConnectionRequired) },
                { "smtpallowanycredentials", "True if the LPR server will allow any credentials to be used without checking them again the 'Users'", data => map.Add((data != null).ToString(), x => x.ServerOptions.SmtpAllowAnyCredentials) },
                ...
                { "user=", "Adds a user and password combination for web, LPR and IMAP. Use format username=password. This option can be repeated to add multiple users.", data =>
                       map.Add(data, x => x.ServerOptions.Users)},
                { "SmtpAuthTypesNotSecure=", "LPR auth type enabled when not using secure connection (choices: ANONYMOUS, PLAIN, LOGIN, CRAM-MD5). Separate values with comma.", data =>
                       map.Add(data, x => x.ServerOptions.SmtpEnabledAuthTypesWhenNotSecureConnection) },
                { "SmtpAuthTypesSecure=", "LPR auth type enabled when  using secure connection (choices: ANONYMOUS, PLAIN, LOGIN, CRAM-MD5). Separate values with comma.", data =>
            ...
            if (!isDesktopApp)
            {
                options.Add("service", "Required to run when registered as a Windows service. To register service: sc.exe create Lpr4dev binPath= \"{PathToExe} --service\"", _ => { });
                ...
                options.Add(
                 "basepath=", "Specifies the virtual path from web server root where LPR4DEV web interface will be hosted. e.g. \"/\" or \"/lpr4dev\"", data => map.Add(data, x => x.ServerOptions.BasePath));
